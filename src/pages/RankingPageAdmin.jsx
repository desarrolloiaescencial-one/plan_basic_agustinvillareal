...
export default function RankingPageAdmin() {
  ...
  async function toggleGlobalDetalle(userId) {
    if (globalDetailUserId === userId) {
      setGlobalDetailUserId(null)
      return
    }
    if (globalDetailApuestas[userId]) {
      setGlobalDetailUserId(userId)
      return
    }
    setGlobalDetailLoading(userId)
    try {
      const r = await sheetsApi.predicciones.apuestasUsuario(userId)
      setGlobalDetailApuestas(prev => ({ ...prev, [userId]: r.apuestas || [] }))
      setGlobalDetailUserId(userId)
    } catch (e) {
      console.error(e.message)
    } finally {
      setGlobalDetailLoading(null)
    }
  }
        ...
        <div className="rk-shell" style={{display:'flex',height:'calc(100vh - 200px)',minHeight:520,borderRadius:20,overflow:'hidden',boxShadow:'0 8px 48px rgba(12,24,43,.14)'}}>
          ...
          <div className="rk-sidebar">
            ...
            <div className="rk-sidebar-scroll">
              <div className={`rk-row${modo==='global'?' sel':''}`} onClick={()=>{setModo('global');setSel(null);setExpandedUser(null)}}>
          ...
          <div className="rk-content" style={{padding:'24px 32px 32px'}}>
            ...
            {modo==='global' ? (
              <VistaGlobal
                ...
                gLoading={gLoading}
                user={user}
                globalDetailUserId={globalDetailUserId}
                globalDetailApuestas={globalDetailApuestas}
                globalDetailLoading={globalDetailLoading}
                onToggleGlobal={toggleGlobalDetalle}
              />
            ) : !sel ? (
              <EmptySelect/>
            ) : (
...
/* ══════════════════════════════════════════
   VISTA GLOBAL (Admin)
══════════════════════════════════════════ */
function VistaGlobal({ gTabla, gMeta, gLoading, user, globalDetailUserId, globalDetailApuestas, globalDetailLoading, onToggleGlobal }) {
      ...
      {gLoading ? (
        ...
        <>
          <Podio
            ...
            expandedUser={globalDetailUserId}
            loadingUser={globalDetailLoading}
          />

          {globalDetailUserId && gTabla.slice(0,3).some(u => u.user_id === globalDetailUserId) && (
            <BetsPanel
              apuestas={globalDetailApuestas[globalDetailUserId] || []}
              loading={globalDetailLoading === globalDetailUserId}
            />
          )}
          ...
          {globalDetailUserId && gTabla.slice(3).some(u => u.user_id === globalDetailUserId) && (
            <BetsPanel
              apuestas={globalDetailApuestas[globalDetailUserId] || []}
              loading={globalDetailLoading === globalDetailUserId}
            />
